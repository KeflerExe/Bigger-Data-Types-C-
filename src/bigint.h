...
class BigInt {
 public:
  ...
  // Conversor de base decimal, octal y hexadecimal a binario
  template <size_t Basa>
  operator BigInt<Basa>() {
    BigInt<Base> bigint_divisor(Basa);
    BigInt<Base> bigint_dividendo;
    BigInt<Base> resto;
    std::string digitos_string;
    bigint_dividendo = *this;
    bigint_dividendo.signo_ = false;
    while (bigint_dividendo >= bigint_divisor) {
      resto = bigint_dividendo % bigint_divisor;
      bigint_dividendo = bigint_dividendo / bigint_divisor;
      if (resto.digitos_.size() == 0) {
        std::string digito_0 = "0";
        digitos_string = digitos_string + digito_0;
      } else {
        if (resto.digitos_[0] == '\000') {
          std::string digito_0 = "0";
          digitos_string = digitos_string + digito_0;
        } else {
          std::string digito_1 = "1";
          digitos_string = digitos_string + digito_1;       
        }
      }
    }
    char ultimo_digito = bigint_dividendo.digitos_[0] + '0';
    std::string ultimo_digito_str(1, ultimo_digito);
    digitos_string = digitos_string + ultimo_digito_str;
    std::reverse(digitos_string.begin(), digitos_string.end());
    char digito_0 = '\000' + '0';
    digitos_string.insert(digitos_string.begin(), digito_0);
    BigInt<2> resultado(digitos_string);
    return resultado;
  }
...
// Especialización de la clase BigInt para números binarios
template <>
class BigInt<2> {
 public:
  // Constructores
  ...
  // Funciones complementarias
  BigInt<2> CalculaElC2() const;
  BigInt<2> EliminaCerosIzquierda() const;
  int TamanioVector() const { return digitos_.size(); }
  int operator[](int posicion) const { return digitos_.at(posicion); }

  // Sobrecarga del operador de asignación
  BigInt<2>& operator=(const BigInt<2>& bigint);

  // Sobrecarga de los operadores lógicos
  ...
  BigInt<2> operator/(const BigInt<2>&) const;
  BigInt<2> operator%(const BigInt<2>&) const;
  BigInt<2> potencia(const BigInt<2>&, const BigInt<2>&) const;

  int ObtieneLaCantidadDeBytes() const;

  // Sobrecarga de los operadores de decremento e incremento
  BigInt<2>& operator++();
  BigInt<2> operator++(int); 
  BigInt<2>& operator--(); 
  BigInt<2> operator--(int); 

  // Conversores de base binaria a base decimal, hexadecimal y decimal
  operator BigInt<8>();
  operator BigInt<10>();
  operator BigInt<16>();
 private:
...
BigInt<Base> BigInt<Base>::operator/(const BigInt<Base>& bigint) const {
  ...
  for (int iterador = 0; iterador < digitos_.size(); ++iterador) {
    if (auxiliar2 < auxiliar) {
      ...
      if (iterador != 0 && cociente.digitos_.size() != 0) {
        cociente.digitos_.insert(cociente.digitos_.begin(), '\000');
      }
    } else {
